...
typedef HeaderWidgetProvider(HeaderModel? value);

class HeaderListWidget extends StatefulWidget with HasFab {
  AppModel app;
  BackgroundModel? listBackground;
  HeaderWidgetProvider? widgetProvider;
  bool? readOnly;
  String? form;
  HeaderListWidgetState? state;
  bool? isEmbedded;

  HeaderListWidget({ Key? key, required this.app, this.readOnly, this.form, this.widgetProvider, this.isEmbedded, this.listBackground }): super(key: key);

  @override
  HeaderListWidgetState createState() {
    state ??= HeaderListWidgetState();
    return state!;
  }
      ...
      onPressed: () {
        Navigator.of(context).push(
          pageRouteBuilder(widget.app, page: BlocProvider.value(
              value: BlocProvider.of<HeaderListBloc>(context),
              child: HeaderForm(app:widget.app,
                  value: null,
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<AccessBloc, AccessState>(
      ...
      if (accessState is AccessDetermined) {
        return BlocBuilder<HeaderListBloc, HeaderListState>(builder: (context, state) {
          if (state is HeaderListLoading) {
            return StyleRegistry.registry().styleWithApp(widget.app).adminListStyle().progressIndicator(widget.app, context);
          } else if (state is HeaderListLoaded) {
    ...
    return Container(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          return HeaderListItem(app: widget.app,
            value: value,
//            app: accessState.app,
  ...
  Widget? getForm(value, action) {
    if (widget.form == null) {
      return HeaderForm(app:widget.app, value: value, formAction: action);
    } else {
      return null;
    }
  }
...
class HeaderListItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Dismissible(
      key: Key('__Header_item_${value.documentID}'),
