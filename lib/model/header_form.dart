  ...
  Widget build(BuildContext context) {
    ...
    if (formAction == FormAction.ShowData) {
      return BlocProvider<HeaderFormBloc >(
            create: (context) => HeaderFormBloc(AccessBloc.appId(context),
                                       formAction: formAction,
      ...
      return Scaffold(
        appBar: StyleRegistry.registry().styleWithContext(context).adminFormStyle().constructAppBar(context, formAction == FormAction.UpdateAction ? 'Update Header' : 'Add Header'),
    ...
    return BlocBuilder<HeaderFormBloc, HeaderFormState>(builder: (context, state) {
      ...
      if (state is HeaderFormInitialized) {
        ...
        children.add(
                  ...
                  StyleRegistry.registry().styleWithContext(context).adminFormStyle().textFormField(context, 'Document ID', Icons.vpn_key, (formAction == FormAction.UpdateAction), _documentIDController, FieldType.String, validator: (_) => state is DocumentIDHeaderFormError ? state.message : null, hintText: 'null')
          );

        children.add(

                  StyleRegistry.registry().styleWithContext(context).adminFormStyle().textFormField(context, 'App Identifier', Icons.text_format, _readOnly(accessState, state), _appIdController, FieldType.String, validator: (_) => state is AppIdHeaderFormError ? state.message : null, hintText: 'This is the identifier of the app to which this feed belongs')
          );

        children.add(

                  StyleRegistry.registry().styleWithContext(context).adminFormStyle().textFormField(context, 'Description', Icons.text_format, _readOnly(accessState, state), _descriptionController, FieldType.String, validator: (_) => state is DescriptionHeaderFormError ? state.message : null, hintText: 'null')
          );
