...
import 'feed_list_event.dart';
import 'feed_list_state.dart';
import 'feed_model.dart';

class FeedComponentSelector extends ComponentSelector {
  @override
  Widget createSelectWidget(BuildContext context, AppModel app, int privilegeLevel, double height,
    ...
    return BlocProvider<FeedListBloc>(
      ...
      child: SelectFeedWidget(app: app,
          height: height,
...
class SelectFeedWidget extends StatefulWidget {
  final AppModel app;
  final double height;
  ...
  @override
  _SelectFeedWidgetState createState() {
    return _SelectFeedWidgetState();
  }
}

class _SelectFeedWidgetState extends State<SelectFeedWidget> with TickerProviderStateMixin {
  ...
  void _handlePrivilegeTabSelection() {
    if ((_privilegeTabController != null) &&
        ...
        BlocProvider.of<FeedListBloc>(context).add(
            FeedChangeQuery(newQuery: getComponentSelectorQuery(_currentPrivilege, widget.app.documentID!)));
    }
  }

  Widget theList(BuildContext context, List<FeedModel?> values) {
    var app = widget.app; 
