...
class FeedJsFirestore implements FeedRepository {
  ...
  Stream<List<FeedModel>> values({String orderBy, bool descending }) {
    if (orderBy == null) {
      ...
      return feedCollection.orderBy(orderBy, descending ? 'desc': 'asc').onSnapshot
          .map((data) => data.docs.map((doc) => _populateDoc(doc)).toList());
    }
  }
  ...
  @override
  Future<List<FeedModel>> valuesList({String orderBy, bool descending }) {
    if (orderBy == null) {
      ...
      return feedCollection.orderBy(orderBy, descending ? 'desc': 'asc').get().then((value) {
        var list = value.docs;
        return list.map((doc) => _populateDoc(doc)).toList();
      });
    }
