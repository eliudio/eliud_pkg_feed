...
typedef FeedMenuWidgetProvider(FeedMenuModel? value);

class FeedMenuListWidget extends StatefulWidget with HasFab {
  BackgroundModel? listBackground;
  FeedMenuWidgetProvider? widgetProvider;
  bool? readOnly;
  String? form;
  FeedMenuListWidgetState? state;
  bool? isEmbedded;

  FeedMenuListWidget({ Key? key, this.readOnly, this.form, this.widgetProvider, this.isEmbedded, this.listBackground }): super(key: key);

  @override
  FeedMenuListWidgetState createState() {
    state ??= FeedMenuListWidgetState();
    return state!;
  }
...
class FeedMenuListWidgetState extends State<FeedMenuListWidget> {
  ...
  Widget? getForm(value, action) {
    if (widget.form == null) {
      return FeedMenuForm(value: value, formAction: action);
    } else {
      return null;
    }
  }
  ...
  Widget build(BuildContext context) {
    return Dismissible(
      ...
      child: ListTile(
        onTap: onTap,
        title: Hero(
          tag: '${value!.documentID}__FeedMenuheroTag',
          child: Container(
